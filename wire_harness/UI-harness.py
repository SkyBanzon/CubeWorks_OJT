...
# ══════════════════════════════════════════════════════════════════════════════
#  TKINTER UI  –  1920x1080, aspect-ratio aware, FIXED SIZE (no growing)
# ══════════════════════════════════════════════════════════════════════════════

def _fit_image(cv_img, box_w, box_h):
    """Resize cv_img to fit box while keeping aspect ratio, centred on black."""
...
class WireMeasurerApp:
    ...
    SIDEBAR_W  = 340
    TOPBAR_H   = 40
    MASK_H     = 180       # mask/skeleton row height
    PAD        = 4
    # Right sidebar split: info panel gets a fixed pixel height,
    # tuning section gets the rest.  Tweak INFO_H to taste.
    INFO_H     = 320       # fixed px height for the measurement image panel
    TUNING_H   = 300       # fixed px height for sliders + checkboxes
    ...
    def __init__(self):
        ...
        # ── Tuning section — fixed-height frame ──
        ...
        tuning_outer = tk.Frame(self.right, bg="#1e1e1e",
                                width=self.SIDEBAR_W - 12,
                                height=self.TUNING_H)
        tuning_outer.pack(fill=tk.X)
        tuning_outer.pack_propagate(False)   # <-- stops tuning section from growing
        ...
        for txt, var, cmd in [("Show Skeleton",       self.skel_var,   None),
                               ("Show Mask/Skel View", self.mask_var,   None),
        ...
        # Top row: Live + Measurement side by side
        ...
        # Bottom row: Mask/Skeleton — fixed height
        self.mask_frame = tk.Frame(self.left, bg="#1e1e1e", height=self.MASK_H)
        self.mask_frame.pack(fill=tk.X, side=tk.BOTTOM, pady=(4, 0))
        self.mask_frame.pack_propagate(False)
        tk.Label(self.mask_frame, text="Mask / Skeleton", bg="#1e1e1e", fg="#aaa",
                 font=("Segoe UI", 9, "bold")).pack(anchor=tk.W)
        self.mask_container = tk.Frame(self.mask_frame, bg="#111")
        self.mask_container.pack(fill=tk.BOTH, expand=True)
        self.mask_container.pack_propagate(False)
        self.mask_lbl = tk.Label(self.mask_container, bg="#111")
        self.mask_lbl.place(x=0, y=0)

        # Deferred initial placeholders
    ...
    def _initial_placeholders(self):
        ...
        self._placeholder(self.mask_container, self.mask_lbl, "")
    ...
    def _show_info(self, cv_img):
        """Render measurement image into the fixed-size info panel."""
        bw = self.SIDEBAR_W - 12
        bh = self.INFO_H
        pil = _fit_image(cv_img, bw, bh)
        photo = ImageTk.PhotoImage(pil)
    ...
    def process_frame(self):
        ...
        ss = self.skel_var.get(); sm = self.mask_var.get(); bm = self.branch_var.get()
        ...
        if hH and self.wsize:
            ...
            if bm:
                ...
                if res is not None:
                    ...
                    tmm = ti['len']*SCALE; self.trunk_buf.append(tmm); ta = float(np.mean(self.trunk_buf))
                    ...
                    for i, br in enumerate(bis):
                        ...
                        if i not in self.br_bufs:
                            self.br_bufs[i] = deque(maxlen=SMOOTH_W); self.tot_bufs[i] = deque(maxlen=SMOOTH_W)
                    ...
                    y0 = 22; nb = len(bis)
                    cv2.putText(info, f"Harness: 1 trunk + {nb} branch(es)", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255,255,255), 1); y0 += 30
                    cv2.putText(info, f"Trunk:  {ta:.1f} mm  ({ta/10:.2f} cm)", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (80,80,255), 1); y0 += 26
                    for i in range(nb):
                        col = BC[i%len(BC)]; ba = float(np.mean(self.br_bufs[i])) if i in self.br_bufs else 0
                        cv2.putText(info, f"Branch {i+1}: {ba:.1f} mm ({ba/10:.2f} cm)", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.45, col, 1); y0 += 22
                    y0 += 6; cv2.line(info, (10,y0), (iw-10,y0), (100,100,100), 1); y0 += 18
                    cv2.putText(info, "Totals (trunk + branch):", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1); y0 += 22
                    for i in range(nb):
                        col = BC[i%len(BC)]; tota = float(np.mean(self.tot_bufs[i])) if i in self.tot_bufs else 0
                        cv2.putText(info, f"Total {i+1}: {tota:.1f} mm ({tota/10:.2f} cm)", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.5, col, 1); y0 += 24
                    cv2.putText(info, f"(avg {len(self.trunk_buf)} frames)", (10, min(y0+6, ih-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (150,150,150), 1)
                else:
                    cv2.putText(info, "No junction (simple fallback)", (10,22), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,165,255), 1)
                    st = simple(sk); y0 = 50
                    for i, (path, lpx) in enumerate(st[:8]):
                        ...
                        cv2.putText(info, f"#{i+1}: {mm:.1f} mm ({mm/10:.2f} cm)", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.45, clr, 1); y0 += 22
            else:
                st = simple(sk); y0 = 22
                cv2.putText(info, "Simple Strand Mode", (10,y0), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1); y0 += 30
                for i, (path, lpx) in enumerate(st[:8]):
                    mm = lpx*SCALE; clr = BC[i%len(BC)]
                    if ss and len(path) > 1:
                        cv2.polylines(disp, [np.array(path, np.int32).reshape(-1,1,2)], False, clr, 2)
            ...
            cv2.putText(disp, "BRANCH" if bm else "SIMPLE", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
            ...
            if sm:
                ...
                self._show(self.mask_container, self.mask_lbl, np.hstack([mb, sb]), "mask")
            else:
                self._placeholder(self.mask_container, self.mask_lbl, "Enable 'Show Mask/Skel View'")
        else:
