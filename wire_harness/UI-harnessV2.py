...
class MaskPopup:
    """Detached Toplevel window for Mask / Skeleton view."""
    ...
    def update(self, cv_img):
        """Push a new OpenCV BGR image into the popup."""
...
class WireMeasurerApp:
    ...
    def __init__(self):
        ...
        self._photo_refs = {}
        self._save_ctr = 0
        self._mask_popup = None   # MaskPopup instance (or None)
        ...
        # ── LEFT views area — now fills full height (no bottom strip) ──
        self.left = tk.Frame(self.body, bg="#1e1e1e")
        self.left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(6, 0), pady=4)

        # Live + Measurement side by side, filling all available space
    ...
    def process_frame(self):
        ...
        if hH and self.wsize:
            ...
            if bm:
                ...
                if res is not None:
                    ...
                    for i, br in enumerate(bis):
                        ...
                        col = BC[i % len(BC)]
                        if ss:
                            # Draw intermediate link segments (junction-to-junction connectors)
                            for lseg in br.get('links', []):
                                lp = lseg['path']
                                if len(lp) > 1:
                                    cv2.polylines(disp, [np.array([(p[1],p[0]) for p in lp], np.int32)], False, col, 3)
                            # Draw the final branch segment
                            bp = br['path']
                            if len(bp) > 1:
            ...
            # Push mask/skel to popup if it's open
            if popup_open:
