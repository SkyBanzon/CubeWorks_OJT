...
SMOOTH_W        = 30
MIN_SEG         = 15
JN_CLUSTER_R    = 20   # Chebyshev radius for clustering junction pixels
JN_MATCH_R      = 25   # max distance to match segment end to junction center
EP_MATCH_R      = 10   # max distance to match segment end to endpoint
...
def homography(corners, ids):
    if not {0,1,2,3}.issubset(set(ids.flatten().tolist())): return None,None
    c={}
    for i,m in enumerate(ids.flatten()):
        p=corners[i][0]; c[int(m)]=np.array([p[:,0].mean(),p[:,1].mean()],np.float32)
    s=np.array([c[0],c[1],c[2],c[3]],np.float32)
    d=np.array([[INSET_PX,INSET_PX],[OUT_W-INSET_PX,INSET_PX],
                [INSET_PX,OUT_H-INSET_PX],[OUT_W-INSET_PX,OUT_H-INSET_PX]],np.float32)
    H,_=cv2.findHomography(s,d); return H,(OUT_W,OUT_H)
...
def analyse(skeleton):
    ...
    # Cluster junction pixels with large radius for thick wires
    ...
    # Cut at junction pixels only (no circle)
    sc=s.copy()
    ...
    for lb in range(1,ncc):
        ...
        if not se: continue
        path=trace(comp,se[0]); length=alen(path)
    ...
    # Trunk = longest with at least one real endpoint
    ...
    # Sort by far-endpoint x for stable colors
    def fx(b):
        p=b['path']
        d0=(p[0][0]-tjn[0])**2+(p[0][1]-tjn[1])**2
        d1=(p[-1][0]-tjn[0])**2+(p[-1][1]-tjn[1])**2
        return (p[0] if d0>d1 else p[-1])[1]
    branches.sort(key=fx)
...
def main():
    ...
    while True:
        ...
        if ids is not None and len(ids)>0:
            ...
            H,ws=homography(corners,ids); hH=H is not None; nm=len(ids)
        ...
        if hH and ws:
            ...
            wm=wire_mask(warped,bs,co,ms,bl,ci); wm=mask_aruco(wm)
            sk=skel(wm)
