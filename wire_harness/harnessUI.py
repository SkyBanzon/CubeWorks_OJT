...
class WireMeasurerApp:
    ...
    def __init__(self):
        ...
        # We'll use a single canvas as the background and place widgets on top
        # But simpler: use frames with pack_propagate(False) and fixed pixel sizes
        ...
        self.info_lbl = tk.Label(self.right, bg="#111")
        ...
        sf = tk.Frame(self.right, bg="#1e1e1e"); sf.pack(fill=tk.X)
        self.sliders = {}
        for name, lo, hi, key in [("Block Size",3,101,"block_size"),("C Offset",0,30,"c_offset"),
                                    ("Morph Size",1,15,"morph_size"),("Blur Size",1,25,"blur_size"),
        ...
        tf = tk.Frame(self.right, bg="#1e1e1e"); tf.pack(fill=tk.X, pady=(6, 2))
    ...
    def _on_resize(self, event=None):
        """Re-render placeholders if not running (so they stay centred)."""
        pass  # active frames auto-update each process_frame
    ...
    def _show_info(self, cv_img):
        """Show info panel in the right sidebar info_lbl."""
        self.info_lbl.update_idletasks()
        bw = self.info_lbl.winfo_width()
        bh = self.info_lbl.winfo_height()
        if bw < 2: bw = 330
        if bh < 2: bh = 400
        pil = _fit_image(cv_img, bw, bh)
        photo = ImageTk.PhotoImage(pil)
        ...
        if hH and self.wsize:
            ...
            if bm:
                ...
                if res is not None:
                    ...
                    cv2.putText(info, f"(avg over {len(self.trunk_buf)} frames)", (10, min(y0+10, ih-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (150,150,150), 1)
                else:
                    cv2.putText(info, "No junction (simple fallback)", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,165,255), 1)
                    st = simple(sk); y0 = 60
